...
static void generate_score_batch(const int16_t q_tile[kTileQ][kHeadDim],
                                 ...
                                 hls::stream<ScorePacket> &score_stream) {
#pragma HLS INLINE off
  const int batch_rows = std::min(kQPairBatchRows, valid_q - batch_base);
...
#pragma HLS PIPELINE II=1
      ...
      int64_t dp0 = 0;
      int64_t dp1 = 0;
      const int row1_idx = row1_valid ? (qi + 1) : qi;
      qk_dotprod_slice_pair(q_tile[qi], q_tile[row1_idx], k_tile[kj], row1_valid, dp0, dp1);
      ...
      ScorePacket pkt{};
      pkt.slot = static_cast<uint8_t>(slot);
      pkt.kj = static_cast<uint8_t>(kj);
      pkt.row1_valid = row1_valid;
      pkt.score0 = score0;
      pkt.score1 = score1;
...
#pragma HLS INLINE off
  const int batch_rows = std::min(kQPairBatchRows, valid_q - batch_base);
  const int pair_count = (batch_rows + kRowPar - 1) / kRowPar;
  const int total_packets = pair_count * valid_k;
  for (int n = 0; n < total_packets; ++n) {
#pragma HLS PIPELINE II=1
    const ScorePacket pkt = score_stream.read();
    const int qi = batch_base + static_cast<int>(pkt.slot) * kRowPar;
    online_softmax_ctx_acc_row_step(pkt.score0, v_tile[pkt.kj], row_m[qi], row_l[qi], row_acc[qi]);
    if (pkt.row1_valid) {
      online_softmax_ctx_acc_row_step(pkt.score1, v_tile[pkt.kj], row_m[qi + 1], row_l[qi + 1], row_acc[qi + 1]);
    }
  }
}
}  // namespace
